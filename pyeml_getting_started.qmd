...
Use pyEML's `set` and `delete` methods to edit EML.
...
- Metapype has good IO methods to read and write xml and json and graph its node tree to console. This will help with automation and (eventual) database integration.  
- Metapype has a well-abstracted `Node` class that is similar to lxml's `Element` class (the class on which pyEML is currently built). This similarity will make re-structuring pyEML to rely on `Node`s relatively straightforward.  
- Metapype has a sibling package called `emlvp` that is built to validate `Node` trees and has EML validation methods and integrated EML xsd files. This will make building validation into pyEML straightforward.  
